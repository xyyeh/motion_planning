...
def get_sample_goals(scene, goalset, goal_idx):
    ...
    display_grasp_num = 8
    selected_index = list(
        np.linspace(0, len(scene.traj.goal_set) - 1, display_grasp_num).astype(np.int)
    ) + [goal_idx]
...
def ycb_special_case(pose_grasp, name):
    if name == "037_scissors":  # only accept top down for edge cases
        z_constraint = np.where(
            (np.abs(pose_grasp[:, 2, 3]) > 0.09)
            * (np.abs(pose_grasp[:, 1, 3]) > 0.02)
            * (np.abs(pose_grasp[:, 0, 3]) < 0.05)
        )
